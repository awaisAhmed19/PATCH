        ...
        with open(file_path, "w", newline="") as f:
            ...
            writer.writerow(
                [
                    ...
                    "80",
                    "http",
                    "Apache Struts vulnerability detected (CVE-2017-5638)",
                    '"Upgrade Apache Struts to a patched version (e.g., 2.3.32, 2.5.10.1, or 2.5.12+). If an immediate upgrade is not feasible, implement a Web Application Firewall (WAF) to filter and block malicious requests targeting the `Content-Type` header, or apply a servlet filter to validate and sanitize input for OGNL expressions."',
                    "Critical",
                    "Pending",
                ]
            )
            writer.writerow(
                [
                    ...
                    '"Upgrade Samba to a patched version that addresses the MS17-010 vulnerability (e.g., Samba 4.6.4, 4.5.10, 4.4.14 or newer). Additionally, it is highly recommended to disable the SMBv1 protocol on the server if it\'s not strictly required by legacy systems, and block SMB traffic (TCP ports 139 and 445) at the firewall from untrusted networks."',
                    "Critical",
                    "Pending",
                ]
            )
            writer.writerow(
                [
                    ...
                    "3389",
                    "ms-wbt-server",
                    "RDP vulnerability (CVE-2023-1234)",
                    '"Apply latest security updates for Windows Server 2016. Restrict RDP access to trusted IPs only."',
                    "High",
                    "Pending",
                ]
            )
            writer.writerow(
                [
                    ...
                    "8080",
                    "tomcat",
                    "Apache Tomcat Information Disclosure (CVE-2022-5678)",
                    '"Upgrade Apache Tomcat to a secure version (e.g., 9.0.70+ or 10.1.0+). Review and harden Tomcat configuration to prevent directory listings and sensitive file exposure."',
                    "Medium",
                    "Patched",
                ]
            )
...
def get_gemini_python_code(prompt):
    ...
    try:
        ...
        if generated_code.startswith("```python") and generated_code.endswith("```"):
            ...
            "```"
        ):  # In case it just gives generic code block
...
def user_side_llm_endpoint():
    try:
        ...
        if not request_data or "question" not in request_data:
            return (
                jsonify(
                    {"error": "Invalid request: 'question' field missing in JSON body"}
                ),
                400,
            )
        ...
        if not generated_python_code:
            return (
                jsonify({"error": "Failed to generate Python code from Gemini."}),
                500,
            )
        ...
        if extracted_data is None:
            return (
                jsonify(
                    {
                        "error": "The generated Python code did not produce any data in 'result_data'."
                    }
                ),
                500,
            )
        if isinstance(extracted_data, str) and extracted_data.startswith("Error:"):
            return (
                jsonify({"error": f"Error during data extraction: {extracted_data}"}),
                500,
            )
        # Check for empty list/dict for "No data found" condition
        if (
            not extracted_data
        ):  # Handles empty list, empty dict, or other "falsy" empty collections
            return (
                jsonify(
                    {"analysis": "No data found", "extracted_data": extracted_data}
                ),
                200,
            )
        ...
        return (
            jsonify(
                {"analysis": final_analysis_response, "extracted_data": extracted_data}
            ),
            200,
        )
        ...
        return (
            jsonify(
                {
                    "error": "GEMINI_API_KEY environment variable not set. Please configure it."
                }
            ),
            500,
        )
